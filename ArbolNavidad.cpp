...
#else //sino se asume que es linux o mac 
void clearScreen(){system("clear");}
void textoSalirAnimacion(){puts("\x1b[34m" "(presione enter para salir de la animacion)" "\x1b[0m");};
// y usa la implementacion de Alan Cox incluyendo la libreria de sys/select.h
...
void menu(char matrizArbol[][COL],int arrayFilas[],int cantidadEsferas[],int arrayCol[],int cantidadEsferasCol[]); //menu de opciones
/*opcion 1*/
...
/*opcion 2*/
void seleccionarFil(int arrayFilas[]);//el usuario selecciona la cantidad y las filas para animar mientras se valida la entrada
...
/*opcion 3*/
void seleccionarCol(int arrayCol[]);//el usuario selecciona la cantidad y las columnas para animar mientras se valida la entrada
...
int validacion();//valida la entrada del usuario y retorna solamente un valor entero
...
int main(){
  ...
  menu(matrizArbol,arrayFilas,cantidadEsferas,arrayCol,cantidadEsferasCol); //se llama al menu con las variables
return 0;
}


/*menu de opciones. ciclado y con una opcion para salir.*/
void menu(char matrizArbol[][COL],int arrayFilas[],int cantidadEsferas[],int arrayCol[],int cantidadEsferasCol[]){
int opc=0;//la variable de opciones a hacer en el switch case del menu
      do{
        ...
        //es solo un printf para una edicion mas facil, pero estan recorridos al principio porque sino dan error en cmd
        printf(VERDE "\
-----------------------Arbol de navidad---------------------------------------\n\
Reproducir la animacion de forma aleatoria en todo el arbol          ....... 1\n\
Reproducir la animacion solo en las lineas del arbol seleccionadas   ....... 2\n\
Reproducir la animacion solo en las columnas del arbol seleccionadas ....... 3\n\
Salir                                                                ....... 4\n\
------------------------------------------------------------------------------\n" COLOR_RESET);
        do{//validador de entrada de 1 a 4
        printf(AZUL "Selecciona una opcion: " COLOR_RESET);
        opc=validacion();//se valida la entrada de datos del usuario
          if(opc<1 || opc>4){
          ...
          case 2:
            seleccionarFil(arrayFilas);//se seleccionan las filas
            llenarMatriz(matrizArbol);//se llena la matriz Arbol con (*)
          ...
          case 3:
            seleccionarCol(arrayCol);//se seleccionan las columnas
            llenarMatriz(matrizArbol);//se llena la matriz Arbol con (*)
...
\tand lots of      in 2023 \n" COLOR_RESET);
  ...
  \t    Merry christmas\n\tand lots of " COLOR_RESET);
          ...
          case 3:printf(MAGENTA "code " COLOR_RESET);
            break;
          default:printf("code "); //color de texto de la terminal (usualmente blanco)
            break;
          }

  printf(CYAN "in 2023 \n" COLOR_RESET);
  }
textoSalirAnimacion();//indicador para salir de la animacion
}//void texto
...
//
//
void seleccionarFil(int arrayFilas[]){
int num,filas;
  for(int i=0;i<REN;i++){//se inicializa la matriz en 0
  arrayFilas[i]=0;
  }//i

   do{//validador de entrada de 1 a 10
        printf(CYAN "introduzca la cantidad de filas para animar (1 a 10 lineas)?: " COLOR_RESET);
        filas=validacion();//se valida la entra de datos del usuario para la variable filas
  ...
  num=validacion();//se valida la entra de datos del usuario para la variable num (que es el num de fila)
        ...
        if(matrizArbol[i][j] == 'O' && (i+1)!=arrayFilas[i]){//si es una esfera y no es de las filas seleccionadas se pintara en un solo orden
              if(contColor==6){contColor=0;}//se reinicia el contador de colores para volver a recorrer los colores
                ...
                switch (contColor) {
                  ...
                  case 4:printf(MAGENTA "%c " COLOR_RESET,matrizArbol[i][j]);
                    break;
                  default:printf("%c ",matrizArbol[i][j]); //color de texto de la terminal (usualmente blanco)
                    break;
...
//
//
void seleccionarCol(int arrayCol[]){
int num,columnas;
  ...
  arrayCol[i]=0;
   ...
   do{//validador de entrada de 1 a 19
        ...
        columnas=validacion();//se valida la entra de datos del usuario para la variable columnas
  ...
  num=validacion();//se valida la entra de datos del usuario para la variable num (que es el num de columna)
    if(num<1 || num>19){//si el numero no esta entre 1 y 19 
      printf(AMARILLO "'%i' no esta entre 1 y 19, intente de nuevo\n" COLOR_RESET,num);
      j--;//el contador regresa para volver a elegir ese numero
    };
